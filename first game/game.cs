...
Color.Tomato
		  ...
		  public static Color Filter(Color color, float Distance)
		  {
			   return new Color((color.R - color.R / LightLevel * Distance) / 255, (color.G - color.G / LightLevel * Distance) / 255, (color.B - color.B / LightLevel * Distance) / 255);
		  }
		  public static void CastRay(float segment)
		  {
			   ...
			   for (float Distance = 1; Distance < LightLevel; Distance += 0.3f)
			   {
					...
					for (int tileIndex = 0; tileIndex < Tiles.numTiles; tileIndex++)
					{
						 if (tileType[tileIndex] != 0 && Tiles.collideRectangle[tileIndex].Contains(drawRay))
						 {
							  ...
							  Filter(tileColors[Tiles.tileType[tileIndex]], Distance)
);
							  return;
						 }
					}
